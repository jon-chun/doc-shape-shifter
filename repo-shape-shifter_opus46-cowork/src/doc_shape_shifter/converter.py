...
from .router import get_backend_chain, UnsupportedConversionError
from .utils.formats import DocFormat, FORMAT_EXTENSION, format_from_string
...
def convert(
    ...
    This is the main entry point for the doc-shape-shifter library.
    ...
    Args:
        ...
        output_path: Path for the output file. If None, auto-generated from input path
                     and target format.
        target_format: Target format string (e.g., "md", "pdf", "docx"). Required if
                       output_path is None or has no extension.
        backend: Force a specific backend name (skip auto-selection).
        fallback: If True (default), try alternate backends on failure.

    Returns:
        ConversionResult with success status, output path, timing, and metadata.

    Raises:
        FileNotFoundError: If input file does not exist.
        ValueError: If target format cannot be determined.
    """
...
def _resolve_target_format(
    ...
    Priority:
        1. Explicit target_format argument
        2. Output path extension
