...
# ============================================================================
# CONVERSION MATRIX
# Derived from the survey's Section 4 matrix, adapted for available backends.
...
def get_backend_chain(source: DocFormat, target: DocFormat) -> list[str]:
    ...
    Args:
        source: Source document format.
        target: Target document format.

    Returns:
        List of backend name strings, ordered from best to fallback.

    Raises:
        UnsupportedConversionError: If no conversion path is defined.
    """
    if source == target:
...
def list_supported_conversions() -> list[tuple[str, str, list[str]]]:
    ...
    Returns:
        List of (source, target, backends) tuples.
    """
    results = []
