...
class BuiltinBackend(BaseBackend):
    ...
    Handles simple transformations that don't need external tools:
    - Markdown → Plain Text (strip markdown syntax)
    - HTML → Plain Text (strip tags)
    - JSON → CSV (flatten records)
    - CSV → JSON (parse records)
    - Plain Text → Markdown (wrap)
    - JSON → Plain Text (pretty-print)
    """
        ...
        try:
            ...
            if handler is None:
                return ConversionResult(
                    ...
                    error_message=f"builtin backend does not support {source_format} → {target_format}",
                )
