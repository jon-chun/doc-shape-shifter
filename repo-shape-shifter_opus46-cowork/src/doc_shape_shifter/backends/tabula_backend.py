...
class TabulaBackend(BaseBackend):
    ...
    - Outputs CSV and JSON directly
    - Good for structured/tabular PDFs

    Limitations:
    - Requires Java runtime
    - Only handles PDF → CSV/JSON
    - Struggles with complex merged cells
    - Optional install: pip install tabula-py + Java
    """
    ...
    def convert(
        ...
        if source_format != "pdf" or target_format not in ("csv", "json"):
            return ConversionResult(
                ...
                error_message=f"Tabula only supports PDF → CSV/JSON, not {source_format} → {target_format}",
            )
