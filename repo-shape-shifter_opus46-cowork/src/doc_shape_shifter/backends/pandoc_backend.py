...
# Pandoc input/output format identifiers
_PANDOC_FORMATS = {
    "pdf": "pdf",
    "docx": "docx",
    "md": "markdown",
...
class PandocBackend(BaseBackend):
    ...
    Pandoc is the "Swiss Army knife" of document conversion, supporting 30+ formats.
    It handles most conversions adequately but may not preserve complex layouts
    as well as specialized tools like Docling.

    Requires: `brew install pandoc` (system-level) and optionally `pip install pypandoc`.
    """

    name = "pandoc"

    def is_available(self) -> bool:
        # Check for pandoc CLI on PATH
        if shutil.which("pandoc") is not None:
            return True
        # Check for pypandoc with bundled pandoc
        try:
    ...
    def convert(
        ...
        if not pandoc_from or not pandoc_to:
        ...
        try:
            # Try pypandoc first (cleaner Python API)
