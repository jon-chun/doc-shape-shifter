...
class TestBuiltinConversions:
    ...
    def test_txt_to_md(self, sample_txt, tmp_path):
        out = tmp_path / "output.md"
        result = convert(sample_txt, out, target_format="md")
        assert result.success
        assert out.exists()
        content = out.read_text()
        assert "plain text" in content

    def test_json_to_txt(self, sample_json, tmp_path):
        out = tmp_path / "output.txt"
        result = convert(sample_json, out, target_format="txt")
        assert result.success
        content = out.read_text()
        assert "Alice" in content

    def test_csv_to_txt(self, sample_csv, tmp_path):
        out = tmp_path / "output.txt"
        result = convert(sample_csv, out, target_format="txt")
        assert result.success
        content = out.read_text()
        assert "Alice" in content
